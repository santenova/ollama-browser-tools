...
export const config = {
  ...
  ollamaEndpoints: ["http://localhost:11434","https://christy-ramentaceous-verbatim.ngrok-free.de","/proxy"],
  model: "qwen3:8b",
  query:"why is the sky blue"
};

const SUMMARIES_INDEX="core-dump";

const INDEX_PREFIX="tactical";
...
class QueryProcessor {
  ...
  async loadSummaries() {
    ...
    const res = await fetch(`${this.esEp}/${SUMMARIES_INDEX}/_search`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async ensureSummariesIndex() {
    const res = await fetch(`${this.esEp}/${SUMMARIES_INDEX}`);
    if (res.status === 200) return true;
    const createRes = await fetch(`${this.esEp}/${SUMMARIES_INDEX}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        mappings: {
  ...
  async runSummarize() {
    ...
    try {
      ...
      const storeRes = await fetch(`${this.esEp}/${SUMMARIES_INDEX}/_doc`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
// Example usage:
const processor = new QueryProcessor(esEp, config.ollamaEndpoints[0], config.model);
processor.setActiveIndex('_all');
processor.loadQueries().then(() => {
   

         
    console.log([processor,processor.getLog()]);

    processor.runSummarize();
