...
export async function calculator(opts: {
  ...
  const availableFunctions: Record<string, (args: { a: number; b: number }) => number> = {
    addTwoNumbers,
    subtractTwoNumbers,
  };
  ...
  if (!res1.ok) throw new Error(`calculator error: ${res1.status}`);
  ...
  if (assistantMsg.tool_calls?.length) {
    ...
    if (!res2.ok) throw new Error(`calculator final error: ${res2.status}`);
