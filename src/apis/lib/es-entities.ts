...
const defaultIndexFor = (entityName) => {
...
export function getEsConfig() {
  const indices = {};
  BASE_ENTITIES.forEach(e => { indices[e.name] = e.defaultIndex; });
...
const BASE_MAPPING = {
  mappings: {
    dynamic_templates: [
      {
        // All string fields: text for full-text search + .keyword for exact/sort
        strings_as_keyword: {
...
const ensureIndex = async (endpoint, index) => {
  try {
    ...
    if (check.status === 404) {
      await fetch(`${endpoint}/${index}`, {
        ...
        body: JSON.stringify(BASE_MAPPING),
      });
    }
  } catch {}
};
...
const translateQuery = (query) => {
  ...
  for (const [key, value] of Object.entries(query)) {
    if (key === '$or') {
      const clauses = value.map(translateQuery);
      should.push(...clauses);
      continue;
    }
    if (key === '$and') {
...
function createEsEntityHandler(entityName, getConfig) {
  const resolve = () => {
    const cfg = getConfig();
    const index = cfg.indices?.[entityName] || defaultIndexFor(entityName);
        ...
        try {
          ...
          if (lastSnapshot) {
            ...
            for (const [id, source] of current) {
              ...
              if (!prev) {
                callback({ id, type: 'create', data: { id, ...source }, timestamp: new Date().toISOString() });
              } else if (JSON.stringify(prev) !== JSON.stringify(source)) {
                callback({ id, type: 'update', data: { id, ...source }, timestamp: new Date().toISOString() });
              }
            }
            // Detect deletes
    ...
    async schema() {
      ...
      try {
        ...
        for (const [field, def] of Object.entries(mapping)) {
          properties[field] = { type: def.type || 'text' };
        }
